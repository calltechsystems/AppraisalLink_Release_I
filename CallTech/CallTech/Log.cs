    ...
    public class Log
    {
        ...
        private async Task<bool> UploadLogToS3(string logFilePath)
        {
            ...
            try
            {
                using (var stream = new FileStream(logFilePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite))
                {
                    ...
                    {
                        Console.WriteLine($"Upload successful: {s3Key}");
                        return true;
                    }
                    else
                    {
                        Console.WriteLine($"Upload failed with status code: {response.HttpStatusCode}");
                        return false;
                    }
                }
            }
        ...
        public async Task WriteLogAndUploadToS3(string message)
        {
            ...
            try
            {
                ...
                if (isUploaded)
                {
                    Console.WriteLine("Log file uploaded successfully.");
                }
                else
                {
                    Console.WriteLine("Log file upload failed.");
                }
            }
            catch (Exception ex)
            {
