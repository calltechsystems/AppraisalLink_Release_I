...
using PayPal;
using PayPal.Api;
using Agreement = PayPal.Api.Agreement;
using Plan = PayPal.Api.Plan;

//using PayPalCheckoutSdk.Orders;

namespace RecuuringPayPal.Controllers;
...
public class RecurringPaymentsController : ControllerBase
{
    ...
    public async Task<IActionResult> Subscribe([FromQuery] int subscriptionId, [FromQuery] int UserId)
    {
        ...
        var apiContext = isSandbox
            ? new APIContext(new OAuthTokenCredential(clientId, clientSecret,
        ...
        if (subscriptionPlan == null) return NotFound("Subscription plan not found.");
        ...
        var plan = new Plan
        {
            ...
            merchant_preferences = new MerchantPreferences
            {
                ...
                initial_fail_amount_action =
                    "CONTINUE", // Set to CONTINUE to continue billing on initial payment failure
                max_fail_attempts = "0"
            }
        };
        ...
        var request =
            new HttpRequestMessage(HttpMethod.Post, "https://api-m.sandbox.paypal.com/v1/billing/subscriptions");
        request.Headers.Add("Authorization",
            "Bearer A21AALCQDSZoJ7Y5-qcATiocbpGoi6rdq3iZ0nTSYrj5Q-7CdBTdKFKJb4keq-zxH1zrZQJn3vmMquXB-o0QSvCGHt5UIn_PQ");
        request.Headers.Add("Cookie",
            "cookie_check=yes; d_id=9014591e55ee406d8c3ef17b4b8afa971704383329212; enforce_policy=gdpr_v2.1; ts=vreXpYrS%3D1799077728%26vteXpYrS%3D1704385128%26vr%3Dd529b1f218c0a6022c7284eaffbc5f5a%26vt%3Dd529b1f218c0a6022c7284eaffbc5f59%26vtyp%3Dnew; ts_c=vr%3Dd529b1f218c0a6022c7284eaffbc5f5a%26vt%3Dd529b1f218c0a6022c7284eaffbc5f59");
        ...
        var nextPaymentDate = DateTime.Now; // Start billing tomorrow
        ...
        var formattedNextPaymentDate = nextPaymentDate.ToString("yyyy-MM-ddTHH:mm:ssZ");
    ...
    public IActionResult HandlePayPalReturn([FromQuery] string token, [FromQuery] string ba_token)
    {
        try
        {
            ...
            var executedAgreement = Agreement.Execute(apiContext, token);

            var agreementId = executedAgreement.id;
            var agreement = Agreement.Get(apiContext, agreementId);

            var agreements = new Agreement {id = agreementId};
            var endDate = DateTime.Now.ToShortDateString();


            var frequency = executedAgreement.plan.payment_definitions[0].frequency;
            ...
            {
                var agreement1 = new DBL.Models.Agreement();
    ...
    public IActionResult SuspendSubscription([FromQuery] string agreementId)
    {
        try
        {
            ...
            Agreement.Suspend(apiContext, agreementId, agreementStateDescriptor);
    ...
    public IActionResult CancelSubscription([FromQuery] string agreementId)
    {
        try
        {
            ...
            if (agreement == null) return NotFound("Agreement not found.");
            var agreementStateDescriptor = new AgreementStateDescriptor();
            ...
            agreement = Agreement.Get(apiContext, agreementId);
