...
public class AppraiserCompanyController : ControllerBase
{
    ...
    public async Task<IActionResult> updateAppraisalCompanyProfile(int AppraiserCompanyID,
        [FromBody] ClsAppraiserCompany updateRequest)
    {
        log.WriteLog("UpdateAppraiserCompany Function started");
        try
        {
            var updatedAppraiserCompany =
                await _appraiserCompany.UpdateAppraiserCompanyAsync(AppraiserCompanyID, updateRequest);
            ...
            var user = _AppraisallandContext.UserInformations.Where(x => x.UserId == AppraiserCompanyID)
                .FirstOrDefault();
            if (user != null)
            {
            ...
            var Appraiser_Details = _AppraisallandContext.UserInformations.Where(x => x.UserId == AppraiserCompanyID)
                .FirstOrDefault();
            return Ok(new
            {
                Message = $"AppraiserCompany with ID {AppraiserCompanyID} updated successfully",
                AppraiserCompany = updatedAppraiserCompany, IsEmail = Appraiser_Details.GetEmail,
    ...
    public async Task<IActionResult> assignPropopertyByAppCompany(ClsAssignProperty clsAssignProperty)
    {
        ...
        try
        {
            var AssignProperty = await _appraiserCompany.AssignProperty(clsAssignProperty);
            if (AssignProperty)
                return Ok("Property assigned successfully.");
            return BadRequest(
                $"it's not Individual Appraiser{clsAssignProperty.Appraiserid} id OR Failed to assign property.");
        }
        catch (Exception ex)
        {
    ...
    public IActionResult updateIsActive(AppraiserIsActiveCls appraiserIsActiveCls)
    {
        ...
        try
        {
            var userDetails = _AppraisallandContext.Appraisers.Where(x =>
                    x.CompanyId == appraiserIsActiveCls.CompanyId && x.Id == appraiserIsActiveCls.AppraiserId)
                .FirstOrDefault();
            if (userDetails != null)
            {
