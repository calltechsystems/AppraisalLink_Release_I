...
public class BrokerController : ControllerBase
{
    ...
    public async Task<IActionResult> updateBrokerProfile(int BrokerId, [FromBody] ClsBrokerUpdateDto updateRequest)
    {
        ...
        try
        {
            ...
            if (updatedBrokerage == null) return NotFound($"Broker not found with ID {BrokerId} or update failed");
            ...
            var Broker_Details = _AppraisallandContext.UserInformations.Where(x => x.UserId == BrokerId)
                .FirstOrDefault();
            return Ok(new
            {
                Message = $"Broker with ID {BrokerId} updated successfully", Brokerage = updatedBrokerage,
    ...
    public async Task<IActionResult> quoteActionByBroker(int QuoteID)
    {
        ...
        {
            var restProperty = 0;
            var userid = _AppraisallandContext.Bids.Where(x => x.BidId == QuoteID).Select(x => x.AppraiserUserId)
                .FirstOrDefault();
            var user_id = userid;
            var Obj_bids = new List<List<Bid>>();
            ...
            if (user_type == 5)
            {
                ...
                if (details == null)
                {
                    ...
                    var Total_Appraisers =
                        _AppraisallandContext.Appraisers.Where(x => x.CompanyId == details.Id).ToList();
            ...
            if (subscription != null)
            {
                ...
                var topUp_properties = _AppraisallandContext.TransactionLogs.Where(x =>
                        x.UserId == user_id && x.EndDate >= DateTime.Today && x.TransactionDetail.Contains("topup"))
                    .ToList();
                foreach (var item in topUp_properties) topUp_property = topUp_property + (short) item.NoOfProperties;
                if (user_type != 5)
                {
                    var UserProperty = _AppraisallandContext.Bids
                        .Where(x => x.AppraiserUserId == user_id && x.Status == 1).ToList();
                ...
                else
                {
                    ...
                    restProperty = topUpTotalProperties + subscription.TotalProperties -
                                   Obj_bids.SelectMany(innerList => innerList).Count();
                }
            }
            else
            {
            ...
            if (restProperty > 0)
            {
                ...
                if (result != null)
                {
                    ...
                    var PropertyDetails = _AppraisallandContext.Properties.Where(x => x.OrderId == OrderId)
                        .FirstOrDefault();
                    ...
                    var acceptedbid = 0;
                    var Obj_bids2 = new List<List<Bid>>();
                    if (user_type != 5)
                    {
                    ...
                    var topUp_property = _AppraisallandContext.Subscriptions
                        .Where(x => x.UserId == user_id && x.TopUpId != null).FirstOrDefault();
                    int topUpTotalProperties = topUp_property?.TotalProperties ?? 0;
                    var t = subscription.TotalProperties - (short) acceptedbid;
                    var transaction = _AppraisallandContext.TransactionLogs.Where(x =>
                        x.UserId == user_id && x.IsActive == true && (x.TransactionDetail.ToLower().Contains("lite") ||
                                                                      x.TransactionDetail.ToLower().Contains("pro") ||
                                                                      x.TransactionDetail.ToLower()
                                                                          .Contains("ultimate"))).FirstOrDefault();
                    ...
                    else
                    {
                        ...
                        var transactions = _AppraisallandContext.TransactionLogs.Where(x =>
                                x.UserId == user_id && x.IsActive == true && x.TransactionDetail.Contains("topup"))
                            .FirstOrDefault();
                        var topUp_properties = _AppraisallandContext.TransactionLogs.Where(x =>
                        ...
                        transactions.UsedProperties =
                            (short) Math.Abs(acceptedbid - (subscription.TotalProperties + transtionproperty));
    ...
    public async Task<IActionResult> quoteReActionByBroker(int QuoteID)
    {
        ...
        try
        {
            ...
            if (Quotes != null)
            {
                ...
                var PropertyDetails = _AppraisallandContext.Properties.Where(x => x.OrderId == Quotes.OrderId)
                    .FirstOrDefault();
                if (PropertyDetails != null)
                {
                    PropertyDetails.IsCompleted = 1;
