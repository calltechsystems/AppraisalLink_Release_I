...
public class QuotesController : ControllerBase
{
    ...
    public IActionResult setQuotes(ClsBid clsBid)
    {
        ...
        try
        {
            var user_id = clsBid.appraiserId;
            var user_type = _context.UserInformations.Where(x => x.UserId == clsBid.appraiserId).Select(x => x.UserType)
                .FirstOrDefault();
            if (user_type == 5)
            {
                ...
                if (details == null)
                {
                    var AppraiserComapny = _context.AppraiserCompanies
    ...
    public IActionResult getAllQuotesByAppraiser()
    {
        ...
        try
        {
            ...
            if (Quotes != null) return Ok(Quotes.Result);
