...
public class ForgotPasswordController : ControllerBase
{
    ...
    public async Task<IActionResult> SendResetToken(string Email)
    {
        ...
        if (await _forgotPasswordService.SendResetTokenAsync(Email)) return Ok("Reset token sent successfully.");
    ...
    public async Task<IActionResult> VerifyResetPassword([FromBody] CLSForgotPassword request)
    {
        ...
        var user_Details = _context.UserInformations
            .Where(p => p.Password == request.NewPassword && p.Email == request.Email.ToLower()).FirstOrDefault();
