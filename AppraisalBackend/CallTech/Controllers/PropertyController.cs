                ...
                try
                {
                  // var Subscriptions= _context.Subscriptions.Where(x => x.UserId == property.UserId).FirstOrDefault();
                    ...
                    if (subscription != null)
                    {
                        ...
                        var topUp_properties = _context.TransactionLogs.Where(x => x.UserId == User_Id && x.EndDate >= DateTime.Today && x.TransactionDetail.Contains("topup")).ToList();
                    ...
                    if(restProperty>0)
                    {
                        ...
                        if (registeredProperty == true)
                        {
                            ...
                            var transaction=_context.TransactionLogs.Where(x=>x.UserId== User_Id && x.IsActive==true && (x.TransactionDetail.ToLower().Contains("lite") ||x.TransactionDetail.ToLower().Contains("pro")|| x.TransactionDetail.ToLower().Contains("ultimate"))).FirstOrDefault();
                            ...
                            else
                            {
                                ...
                                var transactions = _context.TransactionLogs.Where(x => x.UserId == User_Id && x.IsActive == true && x.TransactionDetail.Contains("topup")).FirstOrDefault();
                                var topUp_properties = _context.TransactionLogs.Where(x => x.UserId == User_Id && x.EndDate >= DateTime.Today && x.TransactionDetail.Contains("topup")).ToList();
                                foreach (var item in topUp_properties)
                                {
                                ...
                                transactions.UsedProperties = (short)Math.Abs(count - (subscription.TotalProperties + transtionproperty));
