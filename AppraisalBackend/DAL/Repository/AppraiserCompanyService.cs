...
public class AppraiserCompanyService : IAppraiserCompany
{
    ...
    public async Task<bool> AssignProperty(ClsAssignProperty assignProperty)
    {
        try
        {
            var Assign_Details = _AppraisallandContext.AssignProperties.Where(x =>
                x.Companyid == assignProperty.Companyid && x.Propertyid == assignProperty.Propertyid).FirstOrDefault();
            if (Assign_Details == null)
            {
                var appraiser = _AppraisallandContext.Appraisers.Where(x => x.Id == assignProperty.Appraiserid)
                    .FirstOrDefault();

                if (appraiser.CompanyId != null)
                {
                    var ObjAssign = new AssignProperty();
            ...
            {
                var property_Details = _AppraisallandContext.Properties
                    .Where(x => x.PropertyId == assignProperty.Propertyid).FirstOrDefault();
                var Appraiser_Details = _AppraisallandContext.Appraisers.Where(x => x.Id == assignProperty.Appraiserid)
                    .FirstOrDefault();
                if (Appraiser_Details != null && property_Details != null)
                {
                    var bid_Details = _AppraisallandContext.Bids
                        .Where(x => x.OrderId == property_Details.OrderId &&
                                    x.AppraiserUserId == Appraiser_Details.UserId)
    ...
    public async Task<List<AssignProperty>> GetAllassignProperty(long companyid)
    {
        ...
        if (AssignProperties != null) return AssignProperties;
        return null;
    }
    ...
    public AppraiserCompany GetAppraiserCompany(long UserId)
    {
        ...
        if (appraiserCompany != null) return appraiserCompany;
        return null;
    }
    ...
    {
        var AppraiserCompanyUser =
            _AppraisallandContext.AppraiserCompanies.Where(x => x.UserId == UserId).FirstOrDefault();
    ...
    public bool SendMailassignPropertyByCompany(string email)
    {
        try
        {
            ...
            appraiserMessage +=
                "We're pleased to inform you that a new property has been assigned to you for appraisal. Thank you for your attention to this assignment.\n\n";
            appraiserMessage +=
