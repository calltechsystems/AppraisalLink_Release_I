...
public class IbidService : Ibid
{
    ...
    public async Task<Bid> AppraiserBidAsync(ClsBid clsBid)
    {
        var Appraiser_Id = clsBid.appraiserId;
        int? BidLowerRange = 0;
        int? BidUpperRange = 0;
        ...
        {
            var appraiserCompanyDetails =
                _context.AppraiserCompanies.Where(x => x.UserId == clsBid.appraiserId).FirstOrDefault();
    ...
    public async Task<List<Bid>> getAllAppraiserBidAsync(int OrderId)
    {
    ...
    public Task<List<Bid>> getAppraiserBidbyId(long PropertyID)
    {
        var bid_data = _context.Bids.Where(x => x.OrderId == PropertyID).OrderByDescending(x => x.RequestTime).ToList();
        if (bid_data != null) return Task.FromResult(bid_data);
        return null;
    }

    public Task<Bid> AcceptBidAsync(int bidId)
    {
    ...
    public Task<Bid> DeclineBidAsync(int bidId)
    {
        var data = _context.Bids.Where(x => x.BidId == bidId).FirstOrDefault();
        if (data != null)
        {
    ...
    public async Task<Bid> UpdateBid(long bidiD, Classes.Bid bid)
    {
        try
        {
    ...
    public Task<List<Bid>> getAppraiserBidbyUserID(long Userid)
    {
    ...
    public Bid UpdateStatus(QuoteClass quoteClass)
    {
        try
        {
    ...
    public Task<List<Bid>> getAllQuotesByAppraiser()
    {
    ...
    public async Task<PublishResponse> SendSms(ClsBid clsBid)
    {
        ...
        if (property != null) User_ID = property.UserId;
    ...
    public bool SendEmail(string toEmail, ClsBid clsBid)
    {
        try
        {
            ...
            var appraiserMessage = "Dear Broker's,\n\n";
            appraiserMessage +=
                "We're pleased to share that one of our esteemed appraisers has submitted a new quote for the property you listed.\n\n";
            appraiserMessage += "Appraiser's Quote Details:\n";
            appraiserMessage +=
                $"• Property: [{property.City},{property.StreetName},{property.StreetNumber},{property.ZipCode}]\n";
            appraiserMessage += $"• Appraised Value: [{clsBid.BidAmount}]\n";
            appraiserMessage += $"• Date of Appraisal: [{DateTime.Now}]\n\n";
            appraiserMessage +=
                "Your prompt attention to this quote is appreciated. Please login to our platform to review the details and proceed accordingly.\n\n";
            appraiserMessage +=
                "[Login](<a\"https://appraisal-eta.vercel.app/>) || [Appraisal Link](<a\"https://appraisal-eta.vercel.app/>)\n\n";
            appraiserMessage +=
    ...
    public bool SendMailAcceptQuotes(int bidId)
    {
        try
        {
            ...
            if (Bid_Details != null)
            {
                var email = _context.UserInformations.Where(x => x.UserId == Bid_Details.UserId).Select(x => x.Email)
                    .FirstOrDefault();
                ...
                var appraiserMessage = "Subject: Exciting Update: Your Property Quote Approved!\n\n";
                appraiserMessage += "Dear Appraiser's,\n\n";
                appraiserMessage +=
                    "Your quote for the property has been approved by the broker. To proceed with the next steps, please visit our website to contact the broker.\n";
    ...
    public bool SendMailDeclineQuotes(int bidId)
    {
        try
        {
            ...
            if (Bid_Details != null)
            {
                ...
                var appraiserMessage = "Dear Appraiser's,\n\n";
                appraiserMessage +=
                    $"We regret to inform you that the broker has rejected your quote for the property (order ID:{Bid_Details.OrderId}). Thank you for your time and effort.\n\n";
                appraiserMessage +=
                    "Thank you for being part of our community! If you have any queries, feel free to reach out.\n\n";
                appraiserMessage += "Best regards,\n";
                appraiserMessage += "Support Team\n";
